...
namespace Homework {
    ...
    bool operator==(const Default& lhs, const Default& rhs) {
        return lhs.number == rhs.number;
    }

    // Loo Spaceship <=> operaator
    std::strong_ordering operator<=>(const Default& lhs, const Default& rhs) {
        return lhs.number <=> rhs.number;
    }
    ...
    Tasks assignment3()
    {
        ...
        results.push_back({"Default is destructible", (std::is_destructible_v<Default>)});
        ...
        results.push_back({"Op is destructible", (std::is_destructible_v<Op>)});
